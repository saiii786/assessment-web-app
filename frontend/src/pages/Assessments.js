...
// Questions data - Full from original, with practical-logic-scenarios updated to 3 text-based
const questionsData = {
  aptitude: [
    // Basic (1-13)
  ...
  'programming-fundamentals': [
    ...
    { id: 22, question: "Predict output for leap year function with 2000, 2002, 2008.", options: ["A) All leap", "B) 2000 leap, 2002 not, 2008 leap", "C) None leap", "D) Error"], correct: 1, isCode: true },
  ...
  'practical-logic-scenarios': [
    // Updated: Only 3 text-based scenarios
    { 
      id: 1, 
      question: "Scenario 1: The production database has suddenly halted due to an unknown error, causing the application to crash. Explain your troubleshooting approach in detail, including the tools you would use (e.g., logs, monitoring tools), your step-by-step analysis, and how you would resolve it to minimize downtime.",
      type: 'text'
    },
    { 
      id: 2, 
      question: "Scenario 2: During API integration testing, you're getting intermittent 500 errors from a third-party service. Describe your debugging process, the tools/techniques you'd employ (e.g., Postman, debugging in VS Code), your analytical style, and preventive measures you'd suggest.",
      type: 'text'
    },
    { 
      id: 3, 
      question: "Scenario 3: A critical deployment to staging failed midway, rolling back automatically but leaving inconsistent data in the cache. Outline your root cause analysis, tools (e.g., CI/CD logs, database queries), resolution steps, and how you'd communicate this to the team.",
      type: 'text'
    }
  ]
};
...
function Assessments() {
  const [view, setView] = useState('list'); // 'list', 'quiz', 'resources'
  ...
  const handleSubmit = () => {
    if (currentCategory === 'practical-logic-scenarios') {
      // For text: Check non-empty, no score
      const allFilled = questions.every(q => selectedAnswers[q.id] && selectedAnswers[q.id].trim() !== '');
      if (!allFilled) return alert('Please provide a response for all scenarios.');
      setSubmitted(true);
    } else {
      // For MCQs: Calculate score
  ...
  const isComplete = questions.length > 0 && 
    (currentCategory === 'practical-logic-scenarios' 
      ? questions.every(q => selectedAnswers[q.id] && selectedAnswers[q.id].trim() !== '')
  ...
  const getCategoryDisplayName = (cat) => {
    const names = {
      ...
      'practical-logic-scenarios': 'Practical Logic Scenarios'
    };
    return names[cat] || cat.replace(/-/g, ' ').toUpperCase();
  };

  // Inline Resources content
  const renderResources = () => (
    <>
      <ResourceSection>
        <h3>Aptitude Resources</h3>
        <p>Links: Khan Academy Math Basics | PDF: Profit/Loss Guide</p>
      </ResourceSection>
      <ResourceSection>
        <h3>Programming Fundamentals</h3>
        <p>Links: freeCodeCamp JS | Video: C Syntax Tutorial</p>
      </ResourceSection>
      {/* Add more sections as needed */}
      <ResourceSection>
        <h3>Vocabulary & Soft Skills</h3>
        <p>Resources: Thesaurus.com | Interview Communication Tips PDF</p>
      </ResourceSection>
    </>
  );
    ...
    return (
      <Container>
        <Header>
          ...
          <p>Choose a category to begin. You can access learning resources anytime.</p>
          <ResourcesButton onClick={() => setView('resources')}>Learning Resources (LMS)</ResourcesButton>
        </Header>
        <CategoryList>
          {categories.map(cat => (
            <CategoryButton key={cat} onClick={() => handleCategorySelect(cat)}>
    ...
    return (
      <Container>
        <Header>
          ...
          <p>Access materials for skill-building (expand this section with videos/PDFs).</p>
          <ResourcesButton onClick={() => setView('list')}>Back to Dashboard</ResourcesButton>
        </Header>
        {renderResources()}
      </Container>
    );
  ...
  if (submitted) {
    return (
      <Container>
        <Header>
          <h1>{getCategoryDisplayName(currentCategory)} Complete!</h1>
        </Header>
        {currentCategory === 'practical-logic-scenarios' ? (
          <ScoreContainer>
            <ScoreText>Responses Submitted Successfully!</ScoreText>
            <p>HR can review your detailed explanations below. Great job providing real-time insights!</p>
            <ResponseList>
              {questions.map(q => (
                <div key={q.id}>
                  <h4>Scenario {q.id}: {q.question}</h4>
                  <p><strong>Your Response:</strong> {selectedAnswers[q.id]}</p>
                </div>
              ))}
            </ResponseList>
          </ScoreContainer>
        ...
        <SubmitButton onClick={() => setView('list')} style={{ marginTop: '1rem' }}>Back to Dashboard</SubmitButton>
        <p><em>Next Steps: Review resources, or discuss results with HR.</em></p>
      </Container>
    );
  }
  ...
  return (
    <Container>
      <Header>
        <h1>{getCategoryDisplayName(currentCategory)} Assessment</h1>
        <p>Answer the questions below. For Practical Logic, provide detailed paragraphs.</p>
        <ResourcesButton onClick={() => setView('resources')}>Learning Resources</ResourcesButton>
      </Header>
      <Instructions>
        Complete all {questions.length} questions in {getCategoryDisplayName(currentCategory)}. Good luck!
      </Instructions>
      {questions.map(q => (
        <QuestionContainer key={q.id}>
          ...
          {q.type === 'text' ? (
            <TextArea
              placeholder="Write your detailed response here (approach, tools, analysis)..."
      ...
      <SubmitButton onClick={handleSubmit} disabled={!isComplete}>
        {isComplete ? 'Submit Assessment' : `Complete all ${questions.length} questions (${Object.keys(selectedAnswers).length}/${questions.length})`}
      </SubmitButton>
    </Container>
  );
}
...
//   const [view, setView] = useState('login'); // 'login', 'list', 'quiz', 'resources'
//   const [currentCategory, setCurrentCategory] = useState('aptitude');
//   const [selectedAnswers, setSelectedAnswers] = useState({});
//   const [score, setScore] = useState(null);
//   const [submitted, setSubmitted] = useState(false);
...
//   // Simple login handler (demo: hardcoded creds)
//   const handleLogin = (e) => {
//     e.preventDefault();
...
//           <p>Login to start your assessments.</p>
//         </Header>
//         <LoginContainer>
...
//     { id: 12, question: "Paw : Cat :: Hoof :", options: ["A) Lamb", "B) Horse", "C) Elephant", "D) Tiger"], correct: 1 },
//     { id: 13, question: "Safe : Secure :: Protect :", options: ["A) Lock", "B) Guard", "C) Sure", "D) Conserve"], correct: 1 },
//     { id: 14, question: "An Informal Gathering occurs when a group of people get together in a casual, relaxed manner. Which situation below is the best example of an Informal Gathering?", options: ["A) A debating club meets on the first Sunday morning of every month.", "B) After finding out about his salary raise, Jay and a few colleagues go out for a quick dinner after work.", "C) Meena sends out 10 invitations for a bachelorette party she is giving for her elder sister.", "D) Whenever she eats at a Chinese restaurant, Roop seems to run into Dibya."], correct: 1 },
//     { id: 15, question: "A Tiebreaker is an additional contest carried out to establish a winner among tied contestants. Choose one situation from the options below that best represents a Tiebreaker.", options: ["A) At halftime, the score is tied at 2-2 in a football match.", "B) Serena and Maria have each secured 1 set in the game.", "C) The umpire tosses a coin to decide which team will have bat first.", "D) RCB and KKR each finished at 140 all out."], correct: 3 },
//     { id: 16, question: "The ‘A’ state government has chalked out a plan for the underdeveloped ‘B’ district, where 66% of the funds will be placed in the hands of a committee of local representatives. Courses of action: I. The ‘A’ state government should decide guidelines and norms for the functioning of the committee. II. Other state government may follow similar plan if directed by the Central government.", options: ["A) If only I follows", "B) If only II follows", "C) If either I or II follows", "D) If neither I nor II follows"], correct: 0 },
//     { id: 17, question: "The car dealer found a tremendous response for the new XYZ’s car booking, with long queues of people complaining about the duration of business hours and arrangements. Courses of action: I. People should make their arrangement of lunch and snacks while going for car XYZ’s booking and be ready to spend several hours. II. Arrangement should be made for more booking desks and increase business hours to serve more people in less time.", options: ["A) If only I follows", "B) If only II follows", "C) If either I or II follows", "D) If neither I nor II follows"], correct: 1 },
//     { id: 18, question: "If a legislature decides to fund agricultural subsidy programs, national radio, and a small business loan program, what 2 other programs can they fund?", options: ["A) harbor improvements and school music program", "B) harbor improvements and international airport", "C) hurricane preparedness and international airport", "D) hurricane preparedness and school music program"], correct: 1 },
//     { id: 19, question: "Kevin, Joseph, and Nicholas are 3 brothers. If the following statements are all true, which of them is the youngest? ✓Kevin is the oldest. ✓Nicholas is not the oldest. ✓Joseph is not the youngest.", options: ["A) Joseph", "B) Kevin", "C) Nicholas", "D) Both Joseph and Nicholas"], correct: 2 },
//     { id: 20, question: "According to a schedule, the restaurant’s floors are polished on either…", options: ["A) Tuesday or Wednesday", "B) Tuesday or Thursday", "C) Wednesday or Thursday", "D) Thursday or Friday"], correct: 2 },
//     { id: 21, question: "Statement: Anger is energy, and how to channelize it is in itself a skill. Assumptions: I. Anger need to be channelized. II. Only skillful people can channelize anger to energy.", options: ["A) If only assumption I is implicit.", "B) If only assumption II is implicit.", "C) if either I or II is implicit.", "D) if neither I or II is implicit."], correct: 1 },
//     { id: 22, question: "Statement: Medicine ‘P’ is a drug that is causing ripples in the medical field. Assumptions: I. No other drug is causing ripples in the medical field. II. Medicine ‘P’ is a great drug.", options: ["A) If only assumption I is implicit.", "B) If only assumption II is implicit.", "C) if either I or II is implicit.", "D) if neither I or II is implicit."], correct: 3 },
//     { id: 23, question: "“Use PQR Ceramics tiles to decorate your house” – An advertisement. Assumptions: I. People want to decorate their house. II. Only ceramic tiles can make a house decorative.", options: ["A) If only assumption I is implicit.", "B) If only assumption II is implicit.", "C) if either I or II is implicit.", "D) if neither I or II is implicit."], correct: 0 },
//     { id: 24, question: "Statements: The old order changed yielding place to new. Conclusions: Change is the law of nature. Discard old ideas because they are old.", options: ["A) Only conclusion I follows", "B) Only conclusion II follows", "C) Either I or II follows", "D) Neither I nor II follows"], correct: 0 },
//     { id: 25, question: "Statements: Government has spoiled many top-ranking financial institutions by appointing bureaucrats as Directors of these institutions. Conclusions: Government should appoint Directors of the financial institutes taking into consideration the expertise of the person in the area of finance. The Director of the financial institute should have expertise commensurate with the financial work carried out by the institute.", options: ["A) Only conclusion I follows", "B) Only conclusion II follows", "C) Either I or II follows", "D) Neither I nor II follows"], correct: 4 }
//   ]
// };
...
//   const getCategoryDisplayName = (cat) => {
//     if (cat === 'practical-logic-scenarios') return 'PRACTICAL LOGIC SCENARIOS';
...
//       <Container>
//         <Header>
//           <h1>{getCategoryDisplayName(currentCategory)} Quiz Complete!</h1>
//         </Header>
//         <ScoreContainer>
//           <ScoreText>Your Score: {score} / {questions.length}</ScoreText>
//           <p>{score === questions.length ? 'Perfect score!' : score >= questions.length / 2 ? 'Good job!' : 'Review and retry!'}</p>
...
//       </CategoryTabs>
//       <Instructions>
//         Answer the {questions.length} questions in {getCategoryDisplayName(currentCategory)}. This simulates a real job interview aptitude test. Good luck!
//       </Instructions>
//       {questions.map(q => (
//         <QuestionContainer key={q.id}>
//           <QuestionText>Question {q.id}: {q.question}</QuestionText>
